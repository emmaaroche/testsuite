...
Some configuration options can be fetched from Kubernetes. To install helper services (e.g., Keycloak, Jaeger, MockServer, Redis), see [values-tools.yaml](https://github.com/Kuadrant/helm-charts-olm?tab=readme-ov-file#testing-charts):
...
You can also pass pytest flags to `make` targets using the `flags` environment variable (see [pytest command-line flags](https://docs.pytest.org/en/stable/reference/reference.html#command-line-flags) for more options):
